...
public class RemoteSession {
	...
	public void tick() {
		...
		while ((message = inQueue.poll()) != null) {
			...
			if (processedCount >= maxCommandsPerTick) {
				plugin.getLogger().warning("Over " + maxCommandsPerTick + " commands were queued - deferring "
	...
	protected void handleCommand(String c, String[] args) {
		...
		try {
			...
			if (c.equals("world.getBlock")) {
				...
				//updateBlock(world, loc, Integer.parseInt(args[3]),
					//	(args.length > 4 ? Byte.parseByte(args[4]) : (byte) 0));
				// EntityType.fromId(Integer.parseInt(args[3]))
				// world.spawnEntity(loc, EntityType.CREEPER );
				
				world.spawnEntity(loc, plugin.entityTypeFromId(Integer.parseInt(args[3])));
				...
				Player currentPlayer = getCurrentPlayer(name);
				// get players current location, so when they are moved we will
				// use the same pitch and yaw (rotation)
				...
				// EntityLiving entity =
				// plugin.getEntityLiving(Integer.parseInt(args[0]));
				...
				Player entity = plugin.getEntity(Integer.parseInt(args[0]));
				if (entity != null) {
					// get entity's current location, so when they are moved we
					// will use the same pitch and yaw (rotation)
	...
	private String getBlocks(Location pos1, Location pos2) {
		...
		return blockData.substring(0, blockData.length() > 0 ? blockData.length() - 1 : 0); // We
																							// don't
																							// want
																							// last
																							// comma
	...
	public Player getCurrentPlayer(String name) {
		...
		if (player == null) {
			...
			// otherwise go and get the host player and make that the attached
			// player
			if (player == null) {
	...
	public String locationToRelative(Location loc) {
		return (loc.getX() - origin.getX()) + "," + (loc.getY() - origin.getY()) + "," + (loc.getZ() - origin.getZ());
	}
	...
	/**
	 * from CraftBukkit's
	 * org.bukkit.craftbukkit.block.CraftBlock.blockFactToNotch
	 */
